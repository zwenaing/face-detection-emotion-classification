...
def vgg12():
    ...
    r3_p1 = MaxPooling2D(pool_size=(2, 2), strides=2, padding="valid")(r3_c2)
    ...
    r4_p1 = MaxPooling2D(pool_size=(2, 2), strides=2, padding="valid")(r4_c3)
    ...
    # model.load_weights("../data/vgg12_raw/" + "keras_vgg12.h5")

    datagen = ImageDataGenerator(rotation_range=45, horizontal_flip=True, width_shift_range=0.1, height_shift_range=0.1)
    datagen.fit(train_x)
    train_history = model.fit_generator(datagen.flow(train_x, train_y, batch_size=256), epochs=500, validation_data=(validation_x, validation_y), verbose=1)

    pred = model.predict(test_x, batch_size=len(test_y))
    model.save_weights("keras_vgg12.h5")
